...
export default function Archive() {
  ...
  const fetchPosts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${apiBase()}/api/posts?type=archive`);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "archive", title: formTitle, content: formContent, password: POST_PASSWORD }) });
        ...
        <div style={{ maxWidth: "var(--content-wide)", margin: "0 auto", padding: "0 2.5rem" }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: "6rem 0", textAlign: "center" }}>
              <p style={{ fontFamily: "var(--font-display)", fontSize: "var(--text-lg)", color: "var(--color-text-muted)" }}>{KR ? "등록된 아카이브가 없습니다" : "No archive posts yet"}</p>
            </div>
          ) : (
